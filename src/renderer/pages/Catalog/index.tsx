...
type Tab = 'servicos' | 'pecas' | 'categorias';
...
export function CatalogPage() {
  ...
  const toggleAtivoMutation = useMutation({
    mutationFn: async ({ type, id, ativo }: { type: 'servico' | 'peca' | 'categoria'; id: number; ativo: boolean }) => {
  ...
  const filteredServicos = useMemo(() => {
    ...
    return result;
  }, [servicosData, searchQuery, filterCategoriaId]);
  ...
  const isLoading = tab === 'servicos' ? servicosLoading : tab === 'pecas' ? pecasLoading : categoriasLoading;
  const totalData = tab === 'servicos' ? servicosData : tab === 'pecas' ? pecasData : categoriasData;
  const currentData = tab === 'servicos' ? filteredServicos : tab === 'pecas' ? filteredPecas : filteredCategorias;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-4 border-b">
        {(['servicos', 'pecas', 'categorias'] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => { handleTabChange(t); setSearchQuery(''); setFilterCategoriaId(''); }}
